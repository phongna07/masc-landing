...
import { members, roundTwoSubmissions } from "@masc-landing/db/schema/index";
...
function objectKey(teamId: string, uploadId: string, extension: string) {
  return `round-2/${teamId}/${uploadId}${extension}`;
}
...
export const roundTwoRouter = router({
  ...
  finalize: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [previous] = await db.select({ objectKey: roundTwoSubmissions.objectKey })
        .from(roundTwoSubmissions).where(eq(roundTwoSubmissions.teamId, membership.teamId)).limit(1);
      try {
        await db.insert(roundTwoSubmissions).values({
          ...
          target: roundTwoSubmissions.teamId,
          set: {
            description: input.description,
            objectKey: key,
            originalFilename: input.filename,
