...
export const roundOneRouter = router({
  current: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [submission] = await db
      ...
      .limit(1);
    return { submission: submission ?? null, isSubmissionOpen: settings.roundOneSubmissionOpen };
  }),

  createUploadUrl: protectedProcedure.input(fileInput).mutation(async ({ ctx, input }) => {
    const membership = await membershipFor(ctx.session.user.email);
    requireCaptain(membership);
    await requireSubmissionOpen("roundOne");
  ...
  finalize: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
	  ...
	  try {
		...
		const [current] = await db.select({ objectKey: roundOneSubmissions.objectKey })
