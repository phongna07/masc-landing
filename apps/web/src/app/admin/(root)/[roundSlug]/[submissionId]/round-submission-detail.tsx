...
export default function RoundSubmissionDetail({ round, submissionId }: { round: RoundId; submissionId: string }) {
  ...
  return <><Link className="admin-back-link" href={backHref as Route}><ArrowLeftIcon />{t("actions.backToRound", { round })}</Link>
    ...
    <Card className="dashboard-card admin-submission-card"><CardHeader><CardTitle>{t("round.content")}</CardTitle></CardHeader><CardContent><div className="submission-description"><strong>{t("fields.description")}</strong><p>{data.description}</p></div><div className="submission-file"><FileTextIcon /><div><strong>{data.originalFilename}</strong><span>{formatBytes(data.fileSize, locale)}</span></div><div className="admin-file-actions">{canPreview && <Button variant="outline" disabled={preview.isPending} onClick={() => preview.mutate(input)}><EyeIcon />{t("actions.preview")}</Button>}<Button variant="outline" disabled={download.isPending} onClick={() => download.mutate(input)}><DownloadIcon />{t("actions.download")}</Button></div></div>{fileError && <p className="admin-file-error">{fileError}</p>}{canPreview && previewUrl && <div className="submission-preview"><iframe src={previewUrl} title={t("round.previewTitle", { filename: data.originalFilename })} /></div>}</CardContent></Card>
