...
export default function AdminRoundTwoPage() {
  ...
  const submissions = useQuery(trpc.admin.listRoundTwoSubmissions.queryOptions());

  return <>
    <AdminHeading eyebrow={t("eyebrow")} title={t("roundTwo.title")} description={t("roundTwo.description")} />
    {submissions.isPending ? <AdminLoading /> : submissions.isError ? (
      <AdminError title={t("errors.loadTitle")} description={t("errors.roundTwo")} retry={() => submissions.refetch()} retryLabel={t("actions.retry")} />
    ) : submissions.data.length === 0 ? (
      <AdminEmpty title={t("roundTwo.emptyTitle")} description={t("roundTwo.emptyDescription")} />
    ) : (
      <Card className="admin-table-card"><CardContent className="admin-table-scroll">
        <table className="admin-table admin-round-two-table">
          ...
          <tbody>{submissions.data.map((submission) => <tr key={submission.id}>
            <td><Link className="admin-row-link" href={`/admin/round-two/${submission.id}`}><strong>{submission.teamName}</strong><span>{t(`values.status.${submission.teamStatus}`)}</span></Link></td>
            ...
            <td><Link className="admin-view-link" href={`/admin/round-two/${submission.id}`} aria-label={t("actions.viewSubmission", { team: submission.teamName })}><ChevronRightIcon aria-hidden="true" /></Link></td>
          </tr>)}</tbody>
        </table>
      </CardContent></Card>
    )}
