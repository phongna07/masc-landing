...
export default function RoundSubmissionDetail({ round, submissionId }: { round: RoundId; submissionId: string }) {
  ...
  const [feedback, setFeedback] = useState(""); const [feedbackError, setFeedbackError] = useState<string | null>(null);
  ...
  const data = submission.data; const backHref = `/admin/round-${round}`;
  const submitFeedback = (published: boolean) => { const clean = feedback.trim(); setFeedbackError(null); if (!clean) return setFeedbackError(t("feedback.required")); if (clean.length > 5000) return setFeedbackError(t("feedback.tooLong")); (published ? publish : saveDraft).mutate({ ...input, feedback: clean }); };
  return <><Link className="admin-back-link" href={backHref as Route}><ArrowLeftIcon />{t("actions.backToRound", { round })}</Link>
    <div className="admin-detail-heading"><div><p>{t("round.detailEyebrow", { round })}</p><h1>{data.teamName}</h1></div><span className={`status-badge status-${data.teamStatus}`}>{t(`values.status.${data.teamStatus}`)}</span></div>
    ...
    <Card className="dashboard-card admin-submission-card"><CardHeader><CardTitle>{t("round.content")}</CardTitle></CardHeader><CardContent><div className="submission-description"><strong>{t("fields.description")}</strong><p>{data.description}</p></div><div className="submission-file"><FileTextIcon /><div><strong>{data.originalFilename}</strong><span>{formatBytes(data.fileSize, locale)}</span></div><div className="admin-file-actions"><Button variant="outline" disabled={preview.isPending} onClick={() => preview.mutate(input)}><EyeIcon />{t("actions.preview")}</Button><Button variant="outline" disabled={download.isPending} onClick={() => download.mutate(input)}><DownloadIcon />{t("actions.download")}</Button></div></div>{fileError && <p className="admin-file-error">{fileError}</p>}{previewUrl && <div className="submission-preview"><iframe src={previewUrl} title={t("round.previewTitle", { filename: data.originalFilename })} /></div>}</CardContent></Card>
    <Card className="dashboard-card admin-feedback-card"><CardHeader><CardTitle>{t("feedback.title")}</CardTitle><p>{t(data.feedbackPublished ? "feedback.publishedStatus" : "feedback.draftStatus")}</p></CardHeader><CardContent className="admin-feedback-fields"><Label htmlFor="round-feedback">{t("feedback.label")}</Label><Textarea id="round-feedback" value={feedback} maxLength={5000} rows={8} onChange={(event) => setFeedback(event.target.value)} aria-invalid={!!feedbackError} /><span className="field-hint">{t("feedback.characters", { count: feedback.length })}</span>{feedbackError && <p className="admin-file-error">{feedbackError}</p>}<div className="admin-feedback-actions"><Button variant="outline" disabled={saveDraft.isPending || publish.isPending} onClick={() => submitFeedback(false)}>{t("feedback.saveDraft")}</Button><Button disabled={saveDraft.isPending || publish.isPending} onClick={() => submitFeedback(true)}>{t("feedback.publish")}</Button></div></CardContent></Card>
    <Card className="admin-table-card"><CardHeader><CardTitle>{t("detail.roster")}</CardTitle></CardHeader><CardContent className="admin-table-scroll"><table className="admin-table"><thead><tr><th>{t("fields.member")}</th><th>{t("fields.email")}</th><th>{t("fields.university")}</th><th>{t("fields.role")}</th></tr></thead><tbody>{data.members.map((member) => <tr key={member.id}><td><strong>{member.fullName}</strong></td><td>{member.email}</td><td>{member.universityName}</td><td>{member.isCaptain && <span className="captain-tag">{t("values.captain")}</span>}</td></tr>)}</tbody></table></CardContent></Card>
    <Button className="admin-mobile-back" variant="outline" nativeButton={false} render={<Link href={backHref as Route} />}><ArrowLeftIcon />{t("actions.backToRound", { round })}</Button></>;
}
